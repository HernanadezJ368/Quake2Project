...
RotatePointAroundVector(newpos, ent->pos1, pos, 5); /*So we can rotate around our original line
of sight*/
VectorAdd(ent->pos2, newpos, ent->s.origin); /*then translate the current position back*/

RotatePointAroundVector(ent->velocity, ent->pos1, ent->velocity, 5); /*The velocity vector must be
rotated also*/
...
static void Grenade_Touch4Hyper (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf);
void fire_blaster4Hyper (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, int effect, qboolean hyper)
{
	edict_t	*bolt;
	trace_t	tr;
	...
	bolt->s.sound = gi.soundindex ("misc/lasfly.wav");
	bolt->owner = self;
	bolt->touch = Grenade_Touch4Hyper;
	bolt->nextthink = level.time + 2;
	bolt->think = G_FreeEdict;
	bolt->dmg = damage;
	bolt->classname = "bolt";
...
static void Grenade_Explode (edict_t *ent)
{
	...
	monster->s.origin[2] = ent->s.origin[2]+15;*/
	if (ent->owner->client) //does owner of grenade exist? Is client data defined for entity
		PlayerNoise(ent->owner, ent->s.origin, PNOISE_IMPACT);

	//FIXME: if we are onground then raise our Z just a bit since we are a point?
	if (ent->enemy) //if we explode on someone's head do this
	{
		float	points;
		vec3_t	v;
		vec3_t	dir;
...
static void Grenade_Touch3 (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	vec3_t forward;
	if (other == ent->owner)
		return;
...
static void Grenade_Touch4 (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	vec3_t forward;
	if (other == ent->owner)
		return;
	...
	//void fire_grenade4Gun (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius)
	//fire_grenade (ent, ent->enemy->s.origin, forward, 200, 50, 4, 100);
	Grenade_Explode2 (ent);
}
static int bombbuffer = 1; 
static void Grenade_Touch4Hyper (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	vec3_t forward;
	if (other == ent->owner)
	...
	ent->enemy = other;
	//void fire_grenade4Gun (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius)
	/*if(bombbuffer == 1 || bombbuffer < level.time){
		fire_grenade (ent, ent->enemy->s.origin, forward, 200, 50, 1000, 100);
		bombbuffer = level.time + crandom(40);
	}*/
	Grenade_Explode (ent);
}
...
void fire_grenade4Launcher (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius)
{
	edict_t	*grenade;
	vec3_t	dir;
	vec3_t	forward, right, up;
	...
	VectorMA (grenade->velocity, crandom() * 10.0, right, grenade->velocity);
	VectorSet (grenade->avelocity, 100, 100, 100);
	grenade->movetype = MOVETYPE_FLYMISSILE;
	grenade->clipmask = MASK_SHOT;
	grenade->solid = SOLID_BBOX;
	grenade->s.effects |= EF_IONRIPPER;
	VectorClear (grenade->mins);
	VectorClear (grenade->maxs);
	grenade->s.modelindex = gi.modelindex ("models/objects/gibs/chest/tris.md2");
	grenade->owner = self;
	grenade->touch = Grenade_Touch3;
	grenade->nextthink = level.time + timer;
...
void fire_grenadeTest (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held)
{
	edict_t	*grenade;
	vec3_t	dir;
	vec3_t	forward, right, up;
...
void rocket_touch (edict_t *ent, edict_t *other, cplane_t *plane, csurface_t *surf)
{
	...
	//fire_rocket (ent, ent->s.origin, forward, 200, 100, 50, 200);
	gi.WriteByte (svc_temp_entity);
	if (ent->waterlevel)
		gi.WriteByte (TE_ROCKET_EXPLOSION_WATER);
	else
...
void fire_rocket (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	...
	VectorClear (rocket->mins);
	VectorClear (rocket->maxs);
	rocket->s.modelindex = gi.modelindex ("models/objects/gibs/arm/tris.md2");
	rocket->owner = self;
	rocket->touch = rocket_touch;
...
void fire_leg (edict_t *self, vec3_t start, vec3_t dir, int damage, int speed, float damage_radius, int radius_damage)
{
	edict_t	*rocket;
	rocket = G_Spawn();
	VectorCopy (start, rocket->s.origin);
	...
	rocket->clipmask = MASK_SHOT;
	rocket->solid = SOLID_BBOX;
	rocket->s.effects |= EF_SPINNINGLIGHTS;
	VectorClear (rocket->mins);
	VectorClear (rocket->maxs);
	rocket->s.modelindex = gi.modelindex ("models/objects/gibs/leg/tris.md2");
	rocket->owner = self;
	rocket->touch = Grenade_Touch;
