...
void weapon_grenadelauncher_fire (edict_t *ent)
{
	...
	fire_grenade4Launcher (ent, start, forward, damage, 600, 1, 350);
...
======================================================================
*/
void fire_leg (edict_t *self, vec3_t start, vec3_t aimdir, int damage, int speed, float timer, float damage_radius, qboolean held);
void Weapon_RocketLauncher_Fire (edict_t *ent)
{
	...
	//fire_rocket (ent, ent->s.origin, forward, damage, 50, damage_radius, radius_damage);
	fire_leg (ent, ent->s.origin, forward, damage, 150, 5, damage_radius, false);
	//VectorAdd(ent->s.origin, test, result2);
...
void Blaster_Fire4Shotgun (edict_t *ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	...
	fire_blaster4Shotgun (ent, start, forward, damage, 1000, effect, hyper);
...
void Blaster_Fire4SuperShotgun (edict_t *ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	...
	//fire_blaster4SuperShotgun (ent, start, forward, damage, 1000, effect, hyper);
	//fire_bfg (ent, start, forward, damage, 1000, 20);
	fire_rocket2 (ent, start, forward, 100, 500, 100, 100);
	// send muzzle flash
	gi.WriteByte (svc_muzzleflash);
	gi.WriteShort (ent-g_edicts);
	if (hyper)
...
void Blaster_Fire4Hyper (edict_t *ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	vec3_t	forward, right;
	vec3_t	start;
	vec3_t	offset;
	...
	fire_blaster4Hyper (ent, start, forward, damage, 1000, effect, hyper);
		...
		else
		{
			...
			else
			{
				...
				if (deathmatch->value)
					...
					damage = 20;
				Blaster_Fire4Hyper (ent, offset, 400, true, effect);
				if (! ( (int)dmflags->value & DF_INFINITE_AMMO ) )
					ent->client->pers.inventory[ent->client->ammo_index]--;
...
void weapon_shotgun_fire (edict_t *ent)
{
	...
	ent->client->kick_angles[2] = -5000;
	ent->client->kick_angles[0] = -5000;
	ent->client->kick_angles[1] = -5000;
...
void weapon_supershotgun_fire (edict_t *ent)
{
	...
	fire_SuperShotgun (ent, start, forward, damage, kick, DEFAULT_SHOTGUN_HSPREAD, DEFAULT_SHOTGUN_VSPREAD, DEFAULT_SSHOTGUN_COUNT/2, MOD_SSHOTGUN);
	v[YAW]   = ent->client->v_angle[YAW] + 5;
	AngleVectors (v, forward, NULL, NULL);
	fire_SuperShotgun (ent, start, forward, damage, kick, DEFAULT_SHOTGUN_HSPREAD, DEFAULT_SHOTGUN_VSPREAD, DEFAULT_SSHOTGUN_COUNT/2, MOD_SSHOTGUN);
